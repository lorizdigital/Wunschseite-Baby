...
export function NewWishlistForm() {
  ...
  return <form className="admin-stack" onSubmit={submit}>
    ...
    <label className="admin-field" htmlFor="wishlist-access-code"><span className="admin-field-label">Zugangscode für Familie und Freunde</span><span id="new-wishlist-access-code-explanation" className="field-explanation"><svg aria-hidden="true" viewBox="0 0 24 24"><rect x="5.5" y="10.5" width="13" height="9" rx="2" /><path d="M8.5 10.5V8a3.5 3.5 0 0 1 7 0v2.5" /></svg>Um den Schutz eurer Daten zu gewährleisten, können Personen, denen ihr den Link schickt, nur auf eure Liste zugreifen, wenn sie auch den Code kennen, den ihr hier festlegt.</span><div className="access-code-input-group"><SecretInput id="wishlist-access-code" required minLength={ACCESS_CODE_MIN_LENGTH} maxLength={ACCESS_CODE_MAX_LENGTH} autoComplete="new-password" value={accessCode} aria-describedby="new-wishlist-access-code-explanation new-wishlist-access-code-validation" aria-invalid={accessCode.length > 0 && !accessCodeValidation.valid} onChange={(event) => setAccessCode(event.target.value)} /><p id="new-wishlist-access-code-validation" className={`field-validation field-validation-${accessCodeValidation.kind}`} aria-live="polite">{accessCodeValidation.message}</p></div></label>
