...
export function LoginForm({ nextPath, selfServiceEnabled = false }: { nextPath?: string; selfServiceEnabled?: boolean } = {}) {
  ...
  return <form action={action} className="login-form">
    ...
    <button className="primary-button" disabled={pending}>{pending ? "Wird gesendet …" : selfServiceEnabled ? "Registrieren oder anmelden" : "Anmeldelink senden"}</button>
  </form>;
}
