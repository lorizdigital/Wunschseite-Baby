...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await auth.supabase.rpc("archive_wishlist_v1", { p_wishlist_id: id });
  if (error || !data) return auth.json({ error: "Die Liste konnte nicht archiviert werden." }, 422);
  return auth.json({ archivedAt: data });
}
