...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await auth.supabase.rpc("rotate_wishlist_public_slug_v1", { p_wishlist_id: id });
  if (error || !data) return auth.json({ error: "Der Link konnte nicht erneuert werden." }, 422);
  return auth.json({ publicSlug: data });
}
